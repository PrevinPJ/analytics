...
(couse1 = as.Date("2018-10-25") + 0:29)

#Difference in dates
couse1
length(couse1)  #no of days
min(couse1)  # first date
max(couse1)  # last date
range(couse1) # start to end
mean(couse1)  # center of the course period
median(couse1) # middle date

sd(couse1) #no meaning

#subset / select dates
couse1
#1st and 5th dates
couse1[c(1,5)] # 1st & 5th date 
#1st and 5th dates
couse1[1] ; couse1[5]

#duration from 8th date to first date
(duration1 = couse1[8] - couse1[1])
#total duration
(duration2 = max(couse1) - min(couse1))
